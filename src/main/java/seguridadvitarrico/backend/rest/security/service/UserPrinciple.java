package seguridadvitarrico.backend.rest.security.service;
...
public class UserPrinciple implements UserDetails {
	...
	public static UserPrinciple build(UsuarioEntidad usuario) {
		List<GrantedAuthority> authorities = usuario.getRoles().stream()
				.map(rol -> new SimpleGrantedAuthority(rol.getNombre().name())).collect(Collectors.toList());

		return new UserPrinciple(usuario.getId(), usuario.getNombre(), usuario.getUsuario(), usuario.getCorreo(),
