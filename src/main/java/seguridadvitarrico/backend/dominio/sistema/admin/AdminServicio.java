package seguridadvitarrico.backend.dominio.sistema.admin;
...
import seguridadvitarrico.backend.dominio.sistema.rol.constantes.RolArchivoConstante;
...
import seguridadvitarrico.backend.persistencia.sistema.rol.repositorio.RolEntidadRepositorio;
...
public class AdminServicio {
		...
		public List<String> obtenerRolAdmin() throws InvalidFormatException, IOException {
			OPCPackage file = OPCPackage
					.open(new File(Paths.get("").toAbsolutePath().toString() + RolArchivoConstante.ADMIN_ARCHIVO));
		...
		public void cargarRolAdmin() throws InvalidFormatException, IOException {
			obtenerRolAdmin().forEach(role -> {
